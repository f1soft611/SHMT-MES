...
1. **자동 피드백 루프**: 실행 결과를 평가해 실패 패턴 회피 규칙을 다음 실행에 반영
2. **상태 관리**: 문서 Frontmatter + 세션 메모리로 단계 상태, 실패 이력, 분기 결정을 유지
3. **보수적 동적 분기**: 기본은 1~6 순차, 명시 조건과 대체 검증이 있을 때만 단계 스킵/경량화 허용

### 체인 메타 표준

최종 검증 문서 Frontmatter에는 아래 메타를 기록한다.

- 필수 메타: `workflow_state`, `execution_timestamp`, `input_signature`, `replay_mode`
- 상태 메타: `chain_state_version`, `current_step`, `completed_steps`
- 피드백/분기 메타: `last_error_signature`, `skip_decisions`, `feedback_applied`

### 분기 안전장치

- 분기 전 근거 기록이 없으면 분기 금지
- 스킵 시 대체 검증 항목이 없으면 분기 금지
- 최종 문서에 분기 근거/잔여 위험 누락 시 완료 금지
...
**A**: [페이지네이션*구현*가이드.md](./페이지네이션_구현_가이드.md) 문서 참고
...
**A**: [권한별*읽기쓰기*가이드.md](./권한별_읽기쓰기_가이드.md) 문서 참고
