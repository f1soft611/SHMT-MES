...
- **자동 날짜 감지**: 페이지 재접속 시 날짜가 변경되었으면 자동으로 기본 3일(어제, 오늘, 내일)로 초기화
...
- 요일 체크박스 변경 시 자동 저장
- 다음 방문 시 이전 설정 유지
- 페이지 재접속 시 날짜가 변경되었으면 자동으로 기본 3일로 리셋
- "기본 3일" 버튼으로 수동 리셋 가능
...
- ✅ 전체 표시/전체 숨김 버튼 동작
- ✅ 날짜 변경 시 자동으로 기본 3일로 초기화 (페이지 재접속 시)
- ✅ 캘린더에서 "외 N건" 형식 표시
- ✅ 거래처 칩 클릭 시 전체 목록 팝업
