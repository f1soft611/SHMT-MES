...
- 조회 기간 조건은 `ReqDate`(생산의뢰일, YYYYMMDD) 기준으로 적용
- 스케줄러 입력 기간(`fromDate`, `toDate`)은 `yyyy-MM-dd` 형식이며, 조회 시 `-` 제거 후 `yyyyMMdd`로 변환하여 비교
...
- 생산의뢰 조회 기준일은 `ReqDate`이며 YYYYMMDD (8자리) 형식을 사용
