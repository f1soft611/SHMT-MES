...
const PermissionManagement: React.FC = () => {
  ...
  // 기본 역할 그룹
  const roles = [
    { id: 'GROUP_00000000000000', name: 'ROLE_ADMIN (관리자)' },
    { id: 'GROUP_00000000000001', name: 'ROLE_USER (일반사용자)' },
    { id: 'GROUP_00000000000002', name: 'ROLE_PRODUCTION_MANAGER (생산관리자)' },
    { id: 'GROUP_00000000000003', name: 'ROLE_PRODUCTION_USER (생산사용자)' },
    { id: 'GROUP_00000000000004', name: 'ROLE_VIEWER (조회전용)' },
  ];
  ...
  const loadData = async () => {
    try {
      ...
      if (selectedRole) {
        const rolePermsResult = await permissionService.getRoleMenuPermissions(selectedRole);
  ...
  const handlePermissionChange = async (menuId: string, permissionId: string, granted: boolean) => {
    ...
    try {
      ...
      if (granted) {
        ...
        // 권한 제거
        await permissionService.deleteRoleMenuPermission(selectedRole, menuId, permissionId);
      }
  ...
  const hasPermission = (menuId: string, permissionId: string): boolean => {
    return rolePermissions.some(rp => 
      rp.menuId === menuId && rp.permissionId === permissionId
    );
  };
                ...
                {permissions.map(permission => (
                  <Box key={permission.permissionId} sx={{ mb: 1 }}>
                    <Chip
                      ...
                      color={permission.permissionLevel === 'write' ? 'primary' : 'default'}
                      size="small"
                      sx={{ mr: 1, mb: 1 }}
                    />
                  </Box>
                    ...
                    <Table size="small">
                      <TableHead>
                        <TableRow>
                          ...
                          {permissions.map(permission => (
                            <TableCell key={permission.permissionId} align="center">
                          ...
                          <React.Fragment key={parent.menuId}>
                            <TableRow>
                              ...
                              {permissions.map(permission => (
                                <TableCell key={permission.permissionId} align="center">
                                  <Checkbox
                                    checked={hasPermission(parent.menuId, permission.permissionId)}
                            ...
                            {parent.children?.map(child => (
                              <TableRow key={child.menuId}>
                                ...
                                {permissions.map(permission => (
                                  <TableCell key={permission.permissionId} align="center">
                                    <Checkbox
                                      checked={hasPermission(child.menuId, permission.permissionId)}
                                      onChange={(e) => handlePermissionChange(
                                        child.menuId,
                                        permission.permissionId,
                                        e.target.checked
                      ...
                      <TableRow key={rp.roleMenuId}>
                        ...
                        <TableCell>
                          <Chip
                            ...
                            color={rp.permissionLevel === 'write' ? 'primary' : 'default'}
                            size="small"
                          />
                        </TableCell>
                        <TableCell align="center">
                          <IconButton
                            size="small"
                            onClick={() => handlePermissionChange(rp.menuId, rp.permissionId, false)}
                            disabled={loading}
                          >
                            <DeleteIcon />
                          </IconButton>
