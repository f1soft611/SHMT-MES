...
export default function ProdResultTable({
  rows,
  rowCount,
  pagination,
  onPageChange,
  ...
  return (
    <Card>
      <CardHeader title="생산 목록" titleTypographyProps={{ fontSize: 16 }} />
      <CardContent sx={{ p: 0 }}>
  ...
  return (
    <>
      ...
      <TableRow sx={{ height: 40, cursor: 'pointer' }} onClick={toggleOpen}>
        ...
        <TableCell align="center" sx={{ padding: '0 2px' }}>
          {row.WORKDT_DATE ? dayjs(row.WORKDT_DATE).format('YYYY-MM-DD') : ''}
        </TableCell>
      </TableRow>
        ...
        <TableCell colSpan={13} sx={{ p: 0 }}>
          <Collapse in={open} timeout="auto">
            <Box sx={{ ml: '100px', backgroundColor: '#ddd' }}>
              <ProdResultList ref={detailRef} parentRow={row} />
            </Box>
          </Collapse>
        </TableCell>
      </TableRow>
